...
const ROOrderPaid = 'E2EROPD01';
const ROOrderReceived = 'E2EROREC02';
const ENOrderPaid = 'E2EENPD03';
...
test.describe('Admin orders dashboard', () => {
  test.beforeAll(async () => {
    await Promise.all([
      ...
      seedOrder({
        orderId: ROOrderReceived,
        email: 'ro-received@e2e.test',
        market: 'ro',
        status: 'received',
  ...
  test.afterAll(async () => {
    await Promise.all([
      deleteOrder(ROOrderPaid),
      deleteOrder(ROOrderReceived),
      deleteOrder(ENOrderPaid),
    ]);
    await closeTestDb();
  });
  ...
  test('authenticated list shows seeded orders for both markets', async ({ page }) => {
    await loginAdminInPage(page);
    await page.goto('/admin/orders');
    await expect(page.getByRole('heading', { name: /^Orders$/ })).toBeVisible();

    const tableText = await page.locator('table').first().innerText();
    expect(tableText).toContain(ROOrderPaid);
    expect(tableText).toContain(ROOrderReceived);
    expect(tableText).toContain(ENOrderPaid);
  });

  test('filtering by market=english hides RO orders', async ({ page }) => {
    await loginAdminInPage(page);
    await page.goto('/admin/orders?market=english');
    const tableText = await page.locator('table').first().innerText();
    expect(tableText).toContain(ENOrderPaid);
    expect(tableText).not.toContain(ROOrderPaid);
    expect(tableText).not.toContain(ROOrderReceived);
  });

  test('detail page renders english order with EUR currency', async ({ page }) => {
    await loginAdminInPage(page);
    await page.goto(`/admin/orders/${ENOrderPaid}`);
    await expect(page.getByRole('heading', { name: new RegExp(`Order #${ENOrderPaid}`) })).toBeVisible();
  ...
  test('CSV export contains all seeded orderIds', async ({ request }) => {
    ...
    expect(body).toContain(ROOrderPaid);
    expect(body).toContain(ROOrderReceived);
    expect(body).toContain(ENOrderPaid);
  });
