...
import { clearCartStorage, seedCart, SAMPLE_CART_ITEM, SAMPLE_CART_ITEM_EN } from './fixtures/cart';
import { TEST_SHIPPING } from './fixtures/checkout-data';
import { mockRevolutSDK } from './fixtures/api-mocks';

// Checkout runs on the RO storefront (`/ro/checkout`): the `ro` market enables
// the ramburs (pay-on-delivery) payment method and prices in RON, so the
// seeded RON-priced item survives the CartProvider price guard. The default
// `english` market is card-only, which is why these flows use `/ro`.
...
test.describe('Checkout — Step 1: shipping form', () => {
  ...
  test('shipping step is visible by default', async ({ page }) => {
    await expect(page.getByText(/adresa de livrare/i).first()).toBeVisible();
    await expect(page.locator('#ship-firstName')).toBeVisible();
  });

  test('order summary on the right shows the seeded item', async ({ page }) => {
    await expect(page.getByText(/sumar comandă/i)).toBeVisible();
  ...
  test('submit with empty form shows validation errors and stays on step 1', async ({
    ...
    await expect(page.getByRole('heading', { name: /^💳 plată$/i })).toHaveCount(0);
  });

  test('valid form transitions to payment step', async ({ page }) => {
    await fillShipping(page);
    await page.getByTestId('checkout-continue-payment').click();
    await expect(page.getByRole('heading', { name: /plată/i }).first()).toBeVisible();
    // Compact bullet-list payment UI: a radio for ramburs instead of a
    // stand-alone button. Clicking the radio reveals the
    // "Confirmă comanda ramburs" submit CTA.
    await expect(page.getByTestId('checkout-pay-method-ramburs')).toBeVisible();
  });

  test('Editează button on payment step returns to shipping', async ({ page }) => {
    await fillShipping(page);
    await page.getByTestId('checkout-continue-payment').click();
...
// The Revolut wallet return route (`/revolut-pay/return/*`) is not localized
// and always redirects back to the default-locale `/checkout` (the `english`
// market). Run this restore-draft flow there with the EUR-priced item so the
// CartProvider price guard keeps the cart on return.
test.describe('Checkout — Revolut cancel return (english market)', () => {
  test.beforeEach(async ({ page }) => {
    await clearCartStorage(page);
    await seedStoredEmail(page);
    await seedCart(page, [SAMPLE_CART_ITEM_EN]);
...
test.describe('Checkout — Plata ramburs (with /api/order mocked)', () => {
  test.beforeEach(async ({ page }) => {
    await clearCartStorage(page);
    // Stub the Revolut browser SDK: the fake e2e public key makes the card
    // widget mount its wallet effect on the payment step.
    await mockRevolutSDK(page);
    // Mock /api/order so we don't depend on Resend in the ramburs path. The
  ...
  test('clicking ramburs POSTs to /api/order with the right body', async ({ page }) => {
    ...
    expect(body.paymentMethod).toBe('ramburs');
  ...
  test('successful ramburs redirects to the confirmation page', async ({ page }) => {
    await page.getByTestId('checkout-pay-method-ramburs').click();
    await page.getByTestId('checkout-confirm-ramburs').click();
    await page.waitForURL(/\/(confirmare|order-confirmation)\/[A-Z0-9]+/, { timeout: 10_000 });
    expect(page.url()).toMatch(/\/(confirmare|order-confirmation)\/TESTRMB1$/);
  });

  test('cart is cleared after a successful ramburs order', async ({ page }) => {
    await page.getByTestId('checkout-pay-method-ramburs').click();
    await page.getByTestId('checkout-confirm-ramburs').click();
  ...
  test('successful ramburs order keeps saved customer shipping data', async ({ page }) => {
...
test.describe('Confirmation page', () => {
  test('renders order id and shipping copy for unknown order', async ({ page }) => {
    ...
    const response = await page.goto('/ro/confirmare/MADEUPID');
    expect(response?.status()).toBe(200);
    await expect(page.getByText(/#MADEUPID/i)).toBeVisible();
    await expect(page.getByText(/comanda ta/i).first()).toBeVisible();
  });
    ...
    await page.addInitScript(
      ...
      },
      {
        ...
        value: {
          ...
          email: 'confirm@test.ro',
          phone: '+40740000000',
        },
      },
    );

    await page.goto('/ro/confirmare/MADEUPID');
