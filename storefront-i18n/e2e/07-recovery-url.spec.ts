...
// /recover resolves its market from the request host → localhost is the `ro`
// market, so the recovered item must carry the RO (RON) catalog price or the
// cart page's price guard drops it on hydration.
const SAMPLE_ITEM = {
...
test.describe('Phase 3 — recovery URL flow', () => {
  ...
  test('valid token with seeded cart pins the cookie and lands on the cart page', async ({
    ...
    // The recovery client routes to the market's localized cart page. On
    // localhost the host resolves to the RO market, and since localhost is
    // not a configured market domain the path carries the /ro prefix
    // (see src/app/recover/[token]/page.tsx).
    await page.waitForURL(/\/ro\/comanda$/, { timeout: 20_000 });

    // The recovered items survive the redirect: they were re-priced for the
    // RO market and written to localStorage, and the RO cart page's market
    // price guard keeps them (regression guard for the locale-prefix bug
    // that used to land on /cart (english market) and empty the cart).
