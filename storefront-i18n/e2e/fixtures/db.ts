...
export async function seedCart(args: SeedCartArgs): Promise<void> {
  ...
  await db.collection('carts').insertOne({
    ...
    // localhost resolves to the `ro` market (see src/i18n/market-resolver.ts
    // DEV_HOST_MAP), so seeded carts use the RO locale to match.
    locale: 'ro',
...
export async function seedOrder(args: SeedOrderArgs): Promise<void> {
  const db = await getTestDb();
  const now = new Date();
  const market = args.market ?? 'ro';
  const locale = args.locale ?? (market === 'english' ? 'en' : 'ro');
  const currency = args.currency ?? (market === 'english' ? 'EUR' : 'RON');
  const total = args.totalPrice ?? 1899;
  ...
  await db.collection('orders').insertOne({
    ...
    shippingCost: 0,
    totalPrice: total,
    paymentMethod: args.paymentMethod ?? 'ramburs',
...
/** Cleanup for the review-moderation e2e spec — deletes by slug + name
 *  (the test uses a Date.now()-suffixed name so this never touches real data). */
