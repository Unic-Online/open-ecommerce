...
// + aria-label regex breaks on Romanian pluralization (1 → "produs", 2+ →
// "produse") and on copy edits. The id is part of the component contract.
...
test.describe('Cart sidebar', () => {
  ...
  test('header cart icon reflects total items count', async ({ page }) => {
    ...
    // The aria-label uses Romanian plural rules ("produs"/"produse") so we
    // accept either form.
    await expect(cartButton(page)).toHaveAttribute(
      'aria-label',
      expect.stringMatching(/coș \(3 produse?\)/i),
    );
  });
  ...
  test('clicking the cart icon opens the sidebar with the seeded item', async ({ page }) => {
    ...
    const sidebar = page.locator('aside').filter({ hasText: /coșul tău/i });
    await expect(sidebar).toBeVisible();
    await expect(sidebar).toContainText(SAMPLE_CART_ITEM.productName);
  });

  test('Crește (+) button increments quantity', async ({ page }) => {
    await seedCart(page, [SAMPLE_CART_ITEM]);
    await page.goto('/ro');
    await cartButton(page).click();
  ...
  test('Scade (-) on quantity 1 removes the item', async ({ page }) => {
    await seedCart(page, [SAMPLE_CART_ITEM]);
    await page.goto('/ro');
    await cartButton(page).click();
    ...
    const sidebar = page.locator('aside').filter({ hasText: /coșul tău/i });
    await expect(sidebar.getByText(/coșul tău e gol/i)).toBeVisible();
  });

  test('Șterge button deletes the item', async ({ page }) => {
