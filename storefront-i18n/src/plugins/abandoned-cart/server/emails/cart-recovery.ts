...
const COPY: Record<LocaleKey, Copy> = {
  ro: {
    h1Subject: 'Ai uitat ceva în coș?',
    h1Title: 'Coșul tău te așteaptă',
    h1Preheader: 'Coșul tău e pregătit, un click îl recuperezi.',
    h1Greeting: (n) => (n ? `Bună, ${n}!` : 'Bună!'),
    h1Body:
      'Ai lăsat ceva în coș și ne-am gândit că poate nu vrei să-l pierzi. Ți-am salvat selecția — un singur click și ești înapoi la finalizare.',
    h1Cta: 'Continuă comanda →',
    h24Subject: (p) => `Cod de reducere ${p}% pentru tine`,
    h24Title: 'Reducere exclusivă pentru coșul tău',
    h24Preheader: (code) => `Cod ${code} valabil 7 zile, peste reducerea ta de bun venit.`,
    h24Greeting: (n) => (n ? `${n},` : 'Salut!'),
    h24Body:
      'Coșul tău e încă acolo. Ca să nu ratezi selecția, îți trimitem un cod care se adaugă peste reducerea ta de bun venit:',
    h24CouponEyebrow: 'Codul tău exclusiv',
    h24CouponDescription: (p) => `${p}% în plus la checkout. Valabil 7 zile.`,
    h24Cta: 'Folosește codul →',
    h72Subject: 'Ultima ta șansă la reducere',
    h72Title: 'Codul tău expiră în curând',
    h72Preheader: 'Ultima oară când îți amintim — codul tău expiră curând.',
    h72Body:
      'Ne pare rău să te vedem plecând. E ultima oară când îți amintim de coș. Codul tău rămâne valabil — folosește-l înainte să expire:',
    h72CouponEyebrow: 'Ultimele zile',
    h72CouponDescription: (p) => `+${p}% peste reducerea de bun venit. Codul expiră curând.`,
    h72Cta: 'Recuperează coșul →',
    footerHelpEmail: 'Întrebări? Răspunde direct la acest email — îți răspundem rapid.',
    footerHelpWhatsapp: (w) =>
      `Întrebări? Răspunde direct la acest email sau scrie-ne pe WhatsApp la ${w}.`,
  },
  en: {
...
function itemRowsHtml(items: CartItemData[], market: MarketKey): string {
...
function shellHtml({
  ...
  const c = COPY[marketConfig.locale];
  const helpLine = whatsappDisplay ? c.footerHelpWhatsapp(whatsappDisplay) : c.footerHelpEmail;
  return `
<!DOCTYPE html>
<html>
<head>
...
export function renderRecoveryH1(params: RecoveryEmailParams): RecoveryEmail {
  ...
  const c = COPY[getMarketConfig(market).locale];
    ...
    <tr>
      <td style="padding: 8px 36px 0;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
          ${itemRowsHtml(params.items, market)}
        </table>
      </td>
    </tr>
    <tr>
  ...
  return {
    subject: c.h1Subject,
    html: shellHtml({ title: c.h1Title, preheader: c.h1Preheader, body, market }),
  };
}
