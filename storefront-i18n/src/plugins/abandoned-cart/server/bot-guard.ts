...
//      come from MARKETS so adding a market auto-extends the allowlist.
...
// Derived once at module load. Includes every market's primary domain plus
// every alias, lowercased. The `endsWith` checks below cover same-host
// subdomains (e.g. `staging.shop.example.com`).
const MARKET_ROOT_HOSTS: string[] = (() => {
  const set = new Set<string>();
  for (const m of Object.values(MARKETS)) {
    set.add(m.domain.toLowerCase());
    for (const alias of m.domainAliases ?? []) set.add(alias.toLowerCase());
  }
...
export function isAllowedOrigin(request: Request): boolean {
  ...
  try {
    ...
    // Add a separate marketing/blog apex here if you run one outside MARKETS
