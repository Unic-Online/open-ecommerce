...
export interface UpsertCartArgs {
  ...
  ipAddress?: string;
  userAgent?: string;
  // Phase 3: locale/market the cart is bound to. Optional — defaults to
  // ('ro', undefined) so legacy callers (and pre-Phase-3 docs) keep working.
  // The cart-sync route resolves these from the request host.
  locale?: LocaleKey;
  market?: MarketKey;
}
...
export async function upsertCart(args: UpsertCartArgs): Promise<UpsertCartResult> {
  ...
  const locale: LocaleKey = args.locale ?? 'ro';
