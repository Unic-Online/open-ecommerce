...
export interface CartDoc {
  ...
  items: CartItemData[];
  subtotal: number;
  // Phase 3: locale was previously hardcoded as 'ro'. We widen the type to
  // accept any LocaleKey so the FR market can write 'fr' once the client
  // wires it; cart-sync still writes 'ro' by default for back-compat with
  // existing docs.
  locale: LocaleKey;
