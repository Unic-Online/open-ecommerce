...
export default function ComandaPage() {
  ...
  // Resolve the upsell's price in the active market once, then use it for
  // both display and the cart write below (so the currency symbol and amount
  // always match the active market).
  const upsellMarketPrice = getMarketPrice(`${UPSELL.category}__${UPSELL.slug}`, market.key);
    ...
    return (
      <div className={`${styles.page} container`}>
        <div className={styles.empty}>
          <h1>{t('emptyTitle')}</h1>
          <p>{t('emptyHint')}</p>
          <Link href={'/furniture' as never} className="btn btn-whatsapp">
            {t('viewSafes')}
          </Link>
        </div>
      </div>
