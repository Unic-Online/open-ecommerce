...
function pickLocale(headerValue: string | null): MagicLinkLocale {
  if (headerValue === 'en') return 'en';
  return 'ro';
}
...
export async function POST(request: Request) {
  ...
  const token = createMagicLinkToken(email, locale, nonce);
  const linkUrl = `${originFromRequest(request)}/api/account/verify?token=${encodeURIComponent(token)}`;

  const { subject, html, text } = renderAccountMagicLinkEmail({ linkUrl, locale });
