...
export async function POST(request: Request) {
  try {
    ...
    // Derive commercial market from the request host. Currency, shipping,
    // and checkout gating all derive from market config — the Revolut payment
    // path must be refused on markets where checkout is not enabled, even if
    // the client somehow reaches this endpoint.
    ...
    if (!marketConfig.checkout.enabled) {
      return NextResponse.json(
        { error: 'Plățile cu cardul nu sunt încă disponibile pentru această piață.' },
        { status: 403 },
      )
    }

    // Server-trust: resolve client items against the catalog before pricing.
    // A tampered `unitPrice: 1` for SF01 yields the catalog 1780 RON line.
    ...
    // ramburs the customer is re-doing as card), update it in place. Otherwise
    ...
    try {
      revolutOrder = await createRevolutOrder({
        ...
        currency,
        description: `${marketConfig.name} — comandă #${orderId}`,
        merchantOrderRef: orderId,
        redirectUrl: absoluteUrl(`/confirmare/${orderId}`, market),
        customer: {
          email: shipping.email,
      ...
      return NextResponse.json({ error: 'Nu s-a putut iniția plata cu cardul. Încearcă ramburs sau revino mai târziu.' }, { status: 502 })
    }
