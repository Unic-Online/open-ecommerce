...
// Translation function shape compatible with next-intl's `useTranslations('validation')`.
// Keeping the shape narrow lets the schema accept either a real `t` or a thin
// fallback that maps keys back to the Romanian default messages.
...
// RO defaults — kept identical to the pre-i18n strings so server route
// handlers and any legacy import path keep emitting the original copy.
const RO_DEFAULTS: Record<string, string> = {
  firstNameRequired: 'Prenumele este obligatoriu',
  lastNameRequired: 'Numele este obligatoriu',
  emailInvalid: 'Email invalid',
  phoneInvalid: 'Telefon invalid',
  countyRequired: 'Județul este obligatoriu',
  ...
  companyNameRequired: 'Numele firmei este obligatoriu',
  companyCuiRequired: 'CUI este obligatoriu',
  companyRegComRequired: 'Nr. Reg. Com. este obligatoriu',
  altAddressRequired: 'Adresa de livrare este obligatorie',
  couponInvalid: 'Cod de reducere invalid',
  orderItemsRequired: 'Comanda trebuie să conțină cel puțin un produs',
}

const defaultT: ValidationTranslator = (key) => RO_DEFAULTS[key] ?? key
...
// Locale-aware factory — pass next-intl's `t` from the form to get
...
export const paymentMethodSchema = z.enum(['ramburs', 'card'])
...
export const orderRequestSchema = z.object({
  ...
  paymentMethod: paymentMethodSchema.optional().default('ramburs'),
...
// Product review submission (public form, POST /api/reviews). `slug`
// existence in the product registry is NOT checked here — that pulls in the
// full content/products catalog, which must stay out of the client bundle
// (this schema is shared with the browser form). The route handler checks
// the slug against the registry server-side instead.
