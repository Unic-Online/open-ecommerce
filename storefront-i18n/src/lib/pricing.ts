 ...
 *   - `toMinorUnits` converts stored major-unit prices (whole RON) to bani; ZERO_DECIMAL_CURRENCIES skip the ×100.
...
// Default RO shipping. New markets pass their own ShippingConfig (see Phase 7
// in `i8n_plan.md`); legacy callers with no `shipping` option keep RO behavior.
export const RO_SHIPPING: ShippingConfig = {
...
export interface ComputeOrderOptions {
  ...
  // Per-market shipping config. When omitted, defaults to RO_SHIPPING so
  // existing Romanian-only callers keep their behavior unchanged.
...
export function computeShippingCost(
  subtotal: number,
  shipping: ShippingConfig = RO_SHIPPING,
): number {
...
export function computeOrderTotal<T extends PricedItem>(
  ...
  const shippingCost = computeShippingCost(subtotal, options.shipping ?? RO_SHIPPING);
