...
/**
 * Canonical (default-locale) listing route for a category, e.g. `/furniture`.
 * Used by the Google Merchant feed where the absolute link is built from the
 * default-locale path on the market host.
 */
export function categoryToProductRoute(category: ProductCategory): string {
  const entry = categories.find((c) => c.key === category);
  // `pathnames.en` is the default-locale (canonical, untranslated) form.
  return entry ? entry.pathnames.en : `/${category}`;
}

// Typed pathname literal for the dynamic product route in each category.
// Used with next-intl's `<Link href={{ pathname, params }}>` form so the
// localized URL is generated from routing.ts pathnames map. The pathname keys
// are the canonical (en) category path + `/[slug]`.
 ...
 *
 * Invariants:
 *   - `price`, `oldPrice`, `currency` are RO market values kept here so
 *     legacy callers (Google Merchant feed, related-products grid) keep
 *     working without an explicit market lookup. Server-trusted pricing
 *     for any market goes through `getMarketPrice` in
 *     `src/data/products/prices.ts`.
