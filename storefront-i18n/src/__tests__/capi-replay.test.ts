...
const { mockSendServerPurchase, mockRecordAttempt, mockCaptureError, mockAbsoluteUrl } = vi.hoisted(() => ({
  ...
  mockAbsoluteUrl: vi.fn(
    (path: string, market?: string) => `https://${market ?? 'ro'}.example.test${path}`,
  ),
}))
...
describe('runCapiReplay', () => {
  ...
  it('replays each order with full plumbing: market URL, tracking, experiments, testEventCode', async () => {
    ...
    expect(mockAbsoluteUrl).toHaveBeenCalledWith('/confirmare/AAAA1111', 'english')
    ...
    expect(mockSendServerPurchase).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        numItems: 2,
        eventSourceUrl: 'https://english.example.test/confirmare/AAAA1111',
  ...
  it('falls back to the default market when the order doc has none (pre-Phase-3 docs)', async () => {
    const { db } = makeDb([makeOrder({ market: undefined })])
  ...
  it('passes the persisted order currency through to the Purchase send (EUR stays EUR)', async () => {
    const { db } = makeDb([makeOrder({ market: 'english', currency: 'EUR', totalPrice: 250 })])
  ...
  it('falls back to the market currency when an old doc lacks the currency field', async () => {
    const { db } = makeDb([makeOrder({ market: 'ro', currency: undefined })])
