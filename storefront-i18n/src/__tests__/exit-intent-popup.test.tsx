...
describe('ExitIntentPopup', () => {
  ...
  it('renders the slide-in panel after a top-edge mouseout', () => {
    ...
    fireMouseLeaveTop()
    expect(
      screen.getByRole('dialog', { name: /nu plecai fără coșul tău/i })
    ).toBeInTheDocument()
    expect(screen.getByPlaceholderText(/introdu aici adresa ta de email/i)).toBeInTheDocument()
  })
  ...
  it('skips on conversion routes (/checkout, /comanda, /confirmare/*)', () => {
    for (const p of ['/checkout', '/comanda', '/confirmare/ABCD1234']) {
      setPath(p)
  ...
  it('persists email and shows the success state on submit', async () => {
    ...
    const input = screen.getByPlaceholderText(/introdu aici adresa ta de email/i)
    ...
    expect(await screen.findByText(/mulțumim/i)).toBeInTheDocument()
  ...
  it('rejects an invalid email and stays open', async () => {
    ...
    expect(await screen.findByText(/email validă/i)).toBeInTheDocument()
    expect(localStorage.getItem('sf_user_email')).toBeNull()
  })

  it('renders English copy when the locale is en', () => {
    renderWithProviders(<ExitIntentPopup />, { locale: 'en', market: 'english' })
    fireMouseLeaveTop()
