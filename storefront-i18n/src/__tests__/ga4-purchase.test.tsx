 ...
 *
 * The GA4 property has zero transactions ever: only the Meta Purchase (Pixel +
 * CAPI) fires. These tests assert that the GA4 `purchase` fires wherever the
 * Meta browser Purchase fires — the checkout success path and the confirmation
 * page (wallet-return path) — with:
 *   - `transaction_id` = orderId (same value as the Meta eventID, for
 *     cross-platform reconciliation)
 *   - `value`, per-market `currency`, optional `shipping`
  ...
  items: [
    {
      ...
      productName: 'Consolă din Stejar Aria',
      slug: 'aria-console',
...
describe('Issue #3 — GA4 purchase event', () => {
  ...
  it('uses EUR on the english market', () => {
    trackPurchaseOnce({
      ...PURCHASE_OPTIONS,
      orderId: 'EN123456',
      currency: 'EUR',
    ...
    expect(params.currency).toBe('EUR')
    expect(params.transaction_id).toBe('EN123456')
  })
  ...
  it('confirmation page (wallet-return path) emits GA4 purchase with items and shipping from the order doc', async () => {
    ...
    const ui = await ConfirmarePage({
      params: Promise.resolve({ orderId: 'ABCD1234', locale: 'en' as const }),
    })
    ...
    const params = ga4Purchases()[0][2] as Record<string, unknown>
    expect(params.transaction_id).toBe('ABCD1234')
    expect(params.value).toBe(259)
    expect(params.currency).toBe('EUR')
    expect(params.shipping).toBe(10)
    expect(params.items).toEqual([
      expect.objectContaining({ item_id: 'furniture_aria-console', price: 249, quantity: 1 }),
    ])
