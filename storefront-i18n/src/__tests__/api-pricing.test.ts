...
describe('resolveCartForMarket', () => {
  ...
  it('resolves the english market against the EUR price list', () => {
    const result = resolveCartForMarket([cartItem({})], 'english')
  ...
  it('overrides client-supplied unitPrice with the catalog price', () => {
    // Tampered: client claims oslo-nightstand costs 1 RON.
    const result = resolveCartForMarket(
    ...
    if (result.ok) {
      expect(result.lines).toHaveLength(1)
      // Catalog price for oslo-nightstand is 749 RON.
      expect(result.lines[0].unitPrice).toBe(749)
      expect(result.lines[0].currency).toBe('RON')
...
describe('computeOrderTotal — shipping config', () => {
  ...
  it('defaults to RO shipping (29 RON below the 600 free-shipping threshold)', () => {
    const out = computeOrderTotal([{ quantity: 1, unitPrice: 100 }])
    expect(out.shippingCost).toBe(29)
  })
})

// Smoke test: prove the /api/order route ignores client-supplied unitPrice
// for charging. A tampered oslo-nightstand line at 1 RON must persist a 749 subtotal.
const {
  mockSend,
...
describe('/api/order — server-trusted pricing', () => {
  ...
  it('refuses ramburs orders on the english market (card-only) → 400', async () => {
    // english checkout is enabled but card-only — the /api/order ramburs path
    // must reject any request that tries cash-on-delivery.
    const body = {
      shipping: {
        firstName: 'John',
        lastName: 'Smith',
    ...
    // Server-trust: nothing persisted, no email.
    expect(mockUpsertOrderByCartId).not.toHaveBeenCalled()
    expect(mockSend).not.toHaveBeenCalled()
  })

  it('persists the catalog oslo-nightstand subtotal (749) even when the client sends unitPrice: 1', async () => {
    const body = {
      shipping: {
        firstName: 'Ion',
        lastName: 'Popescu',
    ...
    const res = await orderPost(makeOrderRequest(body))
    ...
    expect(savedItems[0].unitPrice).toBe(749)
  })
})
