 ...
 * resolves the public order token straight to `/confirmare/[orderId]`,
...
// generateMetadata pulls next-intl/server (RSC-only); stub it so the page
// module imports cleanly inside the client-test environment.
...
const PAID_ORDER = {
  ...
  totalPrice: 1124,
  currency: 'RON',
  payment: { provider: 'revolut', currency: 'RON', state: 'completed' },
}

async function renderConfirmation(orderId = 'ABCD1234') {
  const ui = await ConfirmarePage({
    params: Promise.resolve({ orderId, locale: 'ro' as const }),
  })
...
describe('Purchase pixel on the confirmation page (wallet return path)', () => {
  ...
  it('does not fire for an unpaid (pending_payment) order', async () => {
    mockFindOrderById.mockResolvedValue({
      ...
      payment: { provider: 'revolut', currency: 'RON', state: 'pending' },
    })
  ...
  it('checkout success path sets the marker so confirmation will not double-fire', async () => {
    ...
    fireEvent.click(await screen.findByText(/Continuă către plată/))
