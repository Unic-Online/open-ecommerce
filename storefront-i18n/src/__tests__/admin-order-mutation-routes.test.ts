/**
 * Route-level contracts for the admin order mutation endpoints (issue #18):
 * auth gating, body validation, reason→HTTP mapping, and the shipment-email
 * idempotency flow on the fulfillment route. The underlying atomic Mongo
 * contracts (ALLOWED_FROM $in filters, terminal-status $nin, refund bounds,
 * pipeline audit snapshots) are covered at lib level in
 * `orders-mutations.test.ts` — these tests pin how each route exposes them.
 *
 * Uses the shared helpers from `./helpers/` — see issue #20.
 */
...
beforeEach(() => {
  ...
  mockIssueMagicLink.mockResolvedValue('https://ro.shop.example.com/api/account/verify?token=t');
  ...
  it('200 resends and appends an email_resent audit entry WITHOUT touching emailSentAt', async () => {
    ...
    expect(sendEmailMock).toHaveBeenCalledWith(
      expect.objectContaining({
        to: [order.email],
        subject: `Confirmare comandă #${order.orderId}`,
      }),
    );
