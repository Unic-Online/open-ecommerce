...
describe('Revolut webhook route', () => {
  ...
  it('fires server Purchase with the EUR currency persisted on an english-market card order', async () => {
  ...
  it('falls back to the order-market currency when an old doc lacks the currency field', async () => {
    mockFindOrderByProviderOrderId.mockResolvedValueOnce({
      ...LOCAL_ORDER,
      market: 'ro',
      marketingConsent: true,
      currency: undefined,
      // no `currency` on the doc → ro market config → RON
    })
