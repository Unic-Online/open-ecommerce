...
// `data-testid` hooks so e2e specs stop selecting them by Romanian copy.
// These assertions fail on the pre-fix code (no testids) and only pass once
// the attributes are added to the components.
...
describe('checkout/cart interactive elements expose data-testid (#21)', () => {
  ...
  it('tags the shipping and payment step controls', async () => {
    ...
    expect(screen.getByTestId('checkout-pay-method-ramburs')).toBeInTheDocument()

    // Selecting ramburs reveals its confirm button.
    fireEvent.click(screen.getByTestId('checkout-pay-method-ramburs'))
    expect(screen.getByTestId('checkout-confirm-ramburs')).toBeInTheDocument()
  })
})
