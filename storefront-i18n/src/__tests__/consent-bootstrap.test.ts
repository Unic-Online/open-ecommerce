...
describe('Consent Mode + gtag.js ordering (GTM migration invariant)', () => {
  ...
  it('layout.tsx loads GTM via NEXT_PUBLIC_GTM_ID and keeps page_view ownership in GTM', () => {
    // GA4 page_views fire from the config tag inside the GTM container.
    // Issues #2/#3 reintroduced a DELIBERATE direct gtag.js install for the
    // ecommerce events (the container has no per-event GA4 tags, so raw
    // dataLayer pushes would go nowhere) — but it MUST NOT reclaim
    // page_view, or every page would be double-counted. The invariant this
    // guard now pins: GTM still loads, and any direct gtag config disables
    // its automatic page_view.
    ...
    // The ecommerce gtag install must leave page_view to GTM. Scoped to the
    // GA4 measurement-id config: the optional Google Ads (AW-) config
    // deliberately keeps its page ping (it is what builds remarketing
    // audiences and captures gclid; it produces no GA4 page_view).
    const configCalls = layout.match(/gtag\(['"]config['"][^)]*\)/g) ?? []
    const ga4Configs = configCalls.filter((c) => c.includes('analytics.gaId'))
...
describe('Google Ads tag wiring (optional analytics.googleAdsId)', () => {
  const layout = () =>
    readFileSync(join(process.cwd(), 'src/app/[locale]/layout.tsx'), 'utf8')
  ...
  it('layout.tsx configures the Google Ads tag from market config via the shared gtag.js install', () => {
    // Parameterized: the template ships with `analytics.googleAdsId` unset;
    // an operator sets the AW-… id per market in site.config.ts and the
    // shared gtag.js install picks it up — no extra script needed.
    const src = layout()
    const configCalls = src.match(/gtag\(['"]config['"][^)]*\)/g) ?? []
    const adsConfigs = configCalls.filter((c) =>
      c.includes('analytics.googleAdsId'),
    )
  ...
  it('the Ads config keeps its page ping (no send_page_view: false) — remarketing + gclid capture depend on it', () => {
    ...
    const src = layout()
    const configCalls = src.match(/gtag\(['"]config['"][^)]*\)/g) ?? []
    const adsConfig = configCalls.find((c) => c.includes('analytics.googleAdsId'))
  ...
  it('consent bootstrap is installed BEFORE the gtag scripts in the layout (ad_storage gates the Ads tag)', () => {
    ...
    const adsIdx = src.indexOf('analytics.googleAdsId')
