...
describe('CheckoutPage shipping draft', () => {
  ...
  it('restores a saved shipping draft and persists later edits', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Ion'), {
      target: { value: 'Irina' },
    })
  ...
  it('keeps the saved customer data after a successful Revolut payment', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /continuă către plată/i }))
    ...
    expect(mockPush).toHaveBeenCalledWith({ pathname: '/confirmare/[orderId]', params: { orderId: 'ABCD1234' } })
  })
  ...
  it('renders compact payment options and submits the experiment assignment', async () => {
    ...
    expect(screen.getByText(/alege metoda de plată/i)).toBeInTheDocument()
    fireEvent.click(screen.getByRole('radio', { name: /plată ramburs/i }))
    fireEvent.click(screen.getByRole('button', { name: /confirmă comanda ramburs/i }))

    await waitFor(() => {
      expect(mockPush).toHaveBeenCalledWith({ pathname: '/confirmare/[orderId]', params: { orderId: 'RMBR1234' } })
    })
    expect(localStorage.removeItem).not.toHaveBeenCalledWith(CHECKOUT_SHIPPING_DRAFT_KEY)
    expect(localStorage.getItem(CHECKOUT_SHIPPING_DRAFT_KEY)).toEqual(
      expect.stringContaining('"firstName":"Ion"'),
