...
describe('Google Merchant feed — ro market', () => {
  it('includes all catalog products as Google Merchant items', () => {
    const products = getMerchantProducts('ro');
    const feed = generateGoogleMerchantFeed(products, new Date('2026-05-03T10:00:00.000Z'), 'ro');
    ...
    // RON prices for ro market: oslo-nightstand oldPrice=999, price=749
    expect(feed).toContain('<g:price>999.00 RON</g:price>');
    expect(feed).toContain('<g:sale_price>749.00 RON</g:sale_price>');

    // lumen-floor-lamp has no oldPrice → price only, no sale_price element for it
    expect(feed).toContain('<g:price>799.00 RON</g:price>');

    // terra-path-light: oldPrice=449, price=349
    expect(feed).toContain('<g:price>449.00 RON</g:price>');
    expect(feed).toContain('<g:sale_price>349.00 RON</g:sale_price>');
...
describe('Google Merchant feed — english market', () => {
  it('returns a feed for the english market with EUR prices', () => {
    const products = getMerchantProducts('english');
    const feed = generateGoogleMerchantFeed(products, new Date('2026-05-03T10:00:00.000Z'), 'english');

    expect(feed).toContain('xmlns:g="http://base.google.com/ns/1.0"');
    expect((feed.match(/<item>/g) ?? []).length).toBe(products.length);
    expect(products.length).toBe(5);

    // g:brand is english market name
    expect(feed).toContain('<g:brand>Acme Store</g:brand>');

    // EUR prices: oslo-nightstand oldPrice=199 EUR, price=149 EUR
    expect(feed).toContain('<g:price>199.00 EUR</g:price>');
    expect(feed).toContain('<g:sale_price>149.00 EUR</g:sale_price>');

    // lumen-floor-lamp: 159 EUR, no sale
    expect(feed).toContain('<g:price>159.00 EUR</g:price>');

    // terra-path-light: oldPrice=89 EUR, price=69 EUR
