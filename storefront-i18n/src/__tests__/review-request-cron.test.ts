...
describe('runReviewRequestCron', () => {
  ...
  it('sends a market-aware email per eligible order and marks it sent (idempotency marker)', async () => {
  ...
  it('uses locale-appropriate copy for the subject (en vs ro)', async () => {
    const enOrder = deliveredOrder({
      orderId: 'ENDLVR01',
      market: 'english',
      locale: 'en',
      currency: 'EUR',
    });
    const { db } = makeDb([enOrder]);

    await runReviewRequestCron(db, { now: new Date('2026-07-05T00:00:00Z') });

    expect(sendEmailMock.mock.calls[0][0].subject).toMatch(/leave a review/i);
  });
