...
// e.g. card-prepare → switch to ramburs), the coupon was already redeemed on
// the existing doc. The routes correctly reuse `couponDiscountPercent` from
// that doc — but persisted `couponCode` straight from the NEW request body:
...
// host header resolves to the RO market (ramburs enabled); without it the
// request would fall back to DEFAULT_MARKET ('english', card-only).
...
describe('POST /api/order — coupon fields on order reuse', () => {
  it('keeps the redeemed couponCode when the retry body omits it', async () => {
    const res = await postOrder(makeRequest('http://localhost/api/order', orderBody()));
  ...
  it('does not overwrite the redeemed couponCode with a different, never-redeemed code', async () => {
    const res = await postOrder(
      makeRequest('http://localhost/api/order', orderBody({ couponCode: 'SHOP-CCCC-DDDD' })),
    );
    expect(res.status).toBe(200);
...
describe('POST /api/payments/revolut/create-order — coupon fields on order reuse', () => {
  it('keeps the redeemed couponCode when the re-prepare body omits it', async () => {
    const res = await postCreateOrder(
      makeRequest('http://localhost/api/payments/revolut/create-order', orderBody({ paymentMethod: 'card' })),
    );
    expect(res.status).toBe(200);
  ...
  it('does not overwrite the redeemed couponCode with a different code', async () => {
    const res = await postCreateOrder(
      makeRequest(
        'http://localhost/api/payments/revolut/create-order',
