/**
 * Unit tests for the cart-recovery cron engine (issue #17):
 * step transitions, duplicate-send protection (the atomic recoveryStep
 * filter), send-failure rollback, coupon issue/reuse, and the
 * forceAdvanceCart admin guards.
 *
...
vi.mock('@/i18n/market-resolver', () => ({
  absoluteUrl: vi.fn((path: string) => `https://ro.shop.example.com${path}`),
}));
