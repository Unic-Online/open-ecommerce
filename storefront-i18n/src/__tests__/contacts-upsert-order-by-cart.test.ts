...
describe('upsertOrderByCartId', () => {
  ...
  it('updates the existing non-terminal order in place and reuses its orderId', async () => {
    ...
    mockFindOneAndUpdate.mockResolvedValueOnce({ ...existing, paymentMethod: 'ramburs', status: 'received' });

    const result = await upsertOrderByCartId({
      cartId: 'cart-1',
      fallbackOrderId: 'WONTBEUSED',
      email: 'a@b.ro',
      orderData: { paymentMethod: 'ramburs', status: 'received', shipping: {} },
    });
    ...
    expect(set.paymentMethod).toBe('ramburs');
