...
// Mirrors `routing.pathnames` static (non-dynamic) keys. categoryLink.href
// values in `content/products/**` always point at one of these (e.g.
// `/furniture`); we trust the data and narrow at the use site so next-intl's
// strict Link type accepts.
...
export default function ProductBuyBox({ product, reviewSummary }: Props) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      setDeliveryDateLabel(formatDeliveryDate(estimateNextShippingDate(new Date()), locale));
    }, 0);
