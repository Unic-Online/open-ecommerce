...
interface Props {
  ...
  items?: Ga4LineItem[]
  /** Market serving the confirmation page (from the locale segment). */
  market?: MarketKey
}
 ...
 * (`/revolut-pay/return/*` → `/confirmare/[orderId]`), which bypasses
 * checkout's handlePaymentSuccess — without this, only the webhook CAPI
 * Purchase exists for those orders and GA4 would record no transaction.
 *
...
export default function OrderConfirmationPurchaseTracker({
  ...
  useEffect(() => {
    trackPurchaseOnce({ orderId, contentIds, numItems, value, currency, shipping, items, market })
