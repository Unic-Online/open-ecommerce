...
// Single-locale build: SDK locale is always 'en'.
const SDK_LOCALE = 'en' as const;
const WALLET_BUTTON_LOCALE = 'en' as const;
...
export function RevolutPaymentWidgets({
  ...
  const sdkLocale = SDK_LOCALE;
  const walletButtonLocale = WALLET_BUTTON_LOCALE;
    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.cardSection}>
        ...
        {/* Idle: customer hasn't asked for a card session yet — show an explicit
            ...
            payment step but ultimately paid cash-on-delivery (or abandoned). */}
        {shippingValid && cardState === 'idle' && (
          ...
          >
            ...
            <span>{t('payCard', { total: formatMoney(totals.total, market.currency) })}</span>
          </button>
        )}
        {cardState === 'ready' && (
          <button
          ...
          >
            ...
            <span>
              {cardSubmitting ? t('processing') : t('payCard', { total: formatMoney(totals.total, market.currency) })}
            </span>
          </button>
        )}
      </div>
