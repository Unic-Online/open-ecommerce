...
// Compose products so the feed shows the same copy (titles, descriptions,
// gallery) as the rendered product page. Iterates the category registry so a
// new category appears in the feed automatically.
export function getMerchantProducts(): ProductTemplate[] {
  return allCategoryKeys().flatMap((category) => getCategoryProducts({ category }));
}
...
export function generateGoogleMerchantFeed(
  ...
  const resolvedProducts = products ?? getMerchantProducts();
