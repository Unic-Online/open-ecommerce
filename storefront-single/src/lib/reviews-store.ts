 ...
 *     purchased/reviewed), NOT `reviewsKey` — `reviewsKey` is a separate
 *     indirection used only by the curated/static review corpus
 *     (`data/reviews-registry.ts`) for copy reuse across near-identical
 *     products. `getMergedReviews` combines both sources for the product page.
 *     This is the single-language template — there is no locale dimension,
 *     unlike the multi-market `storefront-i18n` sibling.
...
async function ensureReviewsIndexes(): Promise<void> {
  ...
  await c.createIndex({ slug: 1, status: 1, createdAt: -1 });
...
export async function getMergedReviews(
  ...
  const staticReviews = reviewsKey ? getReviewsByLocale(slug) : [];
  const dbReviews = await getApprovedReviews(slug);
