 ...
 *
 * Why: the initial Purchase send (from /api/order for cash-on-delivery, from
 * the Revolut webhook for card) runs inside `after()`. A Vercel cold-freeze, a
 ...
 *     attempts < MAX_ATTEMPTS, and (cod+received) OR (card+paid) —
  ...
  for (const order of eligible) {
    ...
    try {
      ...
      const result = await sendServerPurchase({
        ...
        // the single market's config currency.
        currency: (order.currency as string | undefined) ?? getMarketConfig().currency,
