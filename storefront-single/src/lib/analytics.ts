...
import { categories, MARKET, type ProductCategory } from '@/site.config'
 ...
 * id (`<category>__<slug>`, e.g. `furniture__aria-console`) — the SAME id the
 * Meta events use — and is converted to the Google Merchant Center offerId
 * (`<category>_<slug>`, e.g. `furniture_aria-console`) at send time.
 */
...
export function contentIdToGa4ItemId(contentId: string): string {
  ...
  if (sep > 0) {
    const prefix = contentId.slice(0, sep)
    const slug = contentId.slice(sep + 2)
    const category = categories.find((c) => c.key === prefix)
...
/**
 * Shared GA4 event params: the single market's `currency` plus the `market`
 * and `country` dimensions (for consistent reporting).
 */
function ga4MarketParams(explicitCurrency?: string): Record<string, unknown> {
  return {
    currency: explicitCurrency || MARKET.currency,
    market: MARKET.key,
    country: MARKET.shipping.defaultCountryCode,
  }
}
 ...
 * in `app/layout.tsx`. When gtag.js hasn't loaded yet (it's `lazyOnload`),
 * the standard command-queue stub is installed so early events are replayed
 * by the library once it arrives.
 *
...
export interface ViewContentOptions {
  ...
  contentIds?: string[]
  /** Market currency — defaults to the configured market's currency. */
  currency?: string
}
...
/**
 * Resolve a product detail page from the pathname (`/<category>/<slug>`).
 * Matches against the registered category pathnames so non-product routes
 * with two segments (e.g. `/admin/orders`) never produce a false positive.
 */
export function resolveProductPath(
  ...
  for (const cat of categories) {
    const bases = new Set<string>([`/${cat.key}`, cat.pathname])
...
/**
 * Fire ViewContent for the product page at `pathname` (any category).
 * content_ids uses the catalog id (`<category>__<slug>`, e.g.
 * `furniture__aria-console`) so DPA retargeting joins up with AddToCart /
 * InitiateCheckout / Purchase; value + currency come from the configured
 * market. No-op when the pathname is not a product page.
 */
export function trackProductView(options: { pathname: string; slug?: string }) {
  const resolved = resolveProductPath(options.pathname, options.slug)
  if (!resolved) return

  const product = getSearchableProducts().find(
  ...
  trackViewContent(
    ...
    product.price,
    { contentIds: [contentId], currency: MARKET.currency },
  )
}
...
export interface AddToCartOptions {
  ...
  contentId?: string
  /** Market currency for the GA4 event. */
  currency?: string
}
...
export function trackCartInitiateCheckout(options: CartCheckoutTrackingOptions) {
  ...
  trackEvent('InitiateCheckout', {
    ...
    value: options.value,
    currency: options.currency || MARKET.currency,
