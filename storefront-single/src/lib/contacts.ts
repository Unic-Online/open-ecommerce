 ...
 *
 * - `received` (cod) is included so a customer who placed cod and then
 *   switched back to card lands on the same order doc — but #14 does not
 *   require that direction; the common case is card→cod.
 */
 ...
 *   prepare card → switch to cod → confirm
...
export async function updateOrderPayment(
  ...
  options: {
    ...
    // the customer has converted to cod. The doc's `paymentMethod` is set
    ...
    expectedPaymentMethod?: 'card' | 'cod';
  } = {}
...
export async function claimOrderEmail(
  orderId: string,
  options: { expectedPaymentMethod?: 'card' | 'cod'; staleAfterMs?: number } = {},
