 ...
 * business flags, price, content, embedded reviews — lives in that single
 * object.
 *
 * This is the single-language (English) template: a product carries one flat
 * `content` block and a single `price`/`oldPrice`/`currency` (no per-locale or
 * per-market dimensions).
 *
 * Adapters in `src/lib/catalog.ts`, `src/data/products/{catalog,prices}.ts`,
 * `src/lib/product-search-index.ts`, and the reviews registry read
 * DefinedProduct fields and project them into the existing `ProductTemplate` /
 * `ProductBusiness` / `ProductMarketPrice` shapes — so downstream consumers
 * don't change.
 *
 * Invariants:
 *   - `price` is the source of truth the server charges.
...
export type ProductContentInput = z.infer<typeof contentSchema>;
...
/** Build a legacy `ProductTemplate`-shaped stub (business + content copy). */
...
export function toContent(p: DefinedProduct): ProductContent {
  const c = p.content;
  return {
...
export function getProductReviews(p: DefinedProduct): Review[] {
  return (p.content.reviews ?? []) as Review[];
}
