...
const c: Copy = {
  ...
  introCod: 'Thank you for your order! We\'re getting it ready now.',
...
/** Send path the subject is for: order placed (cod), payment confirmed
 *  (Revolut webhook), or an admin re-send of the confirmation. */
export type CustomerOrderEmailSubjectKind = 'placed' | 'paid' | 'resend'

const SUBJECTS: Record<CustomerOrderEmailSubjectKind, (orderId: string) => string> = {
...
/**
 * Subject line for every customer order-email send path. Routes MUST use
 * this instead of inline literals — the webhook used to send hardcoded
 * Romanian subjects on this English-only template.
 */
export function customerOrderEmailSubject(
  kind: CustomerOrderEmailSubjectKind,
  orderId: string,
): string {
  return SUBJECTS[kind](orderId)
}
...
export function renderCustomerOrderEmail({
  ...
  const money = (amount: number) => formatMoney(amount, marketConfig.currency)
  ...
  const paymentLabel =
    paymentMethod === 'card'
      ...
      : `<span style="color:#b07a00;font-weight:600;">${c.cod}</span>`

  const intro = paymentMethod === 'card' ? c.introCard : c.introCod
