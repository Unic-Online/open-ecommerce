...
export function renderOrderEmail({
  ...
  const paymentLabel = paymentMethod === 'card'
    ? '<strong style="color: #39FF88;">Card (Revolut) — paid ✓</strong>'
    : '<strong style="color: #fbbf24;">Cash on delivery</strong>'
  const shippingLabel = shippingCost === 0 ? 'Free' : formatPrice(shippingCost)
  ...
  <body style="margin: 0; padding: 0; background: #040810; color: #e8ecf2; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
    <div style="max-width: 600px; margin: 0 auto; padding: 32px 20px;">
      ...
      <div style="text-align: center; margin-bottom: 32px;">
        ...
        <p style="color: #8892a6; font-size: 14px; margin: 8px 0 0;">New order received</p>
      </div>
      ...
      <div style="background: #0d1117; border: 1px solid #1a1f2e; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
        <table style="width: 100%;" cellpadding="0" cellspacing="0">
          <tr>
            <td>
              <span style="color: #8892a6; font-size: 12px; text-transform: uppercase; letter-spacing: 0.1em;">Order number</span><br/>
            ...
            <td style="text-align: right;">
              ...
              <strong style="color: #00E5CC; font-size: 24px;">${formatPrice(totalPrice)}</strong>
            </td>
          </tr>
        </table>
      </div>
      ...
      <div style="background: #0d1117; border: 1px solid #1a1f2e; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
        <h2 style="font-size: 16px; margin: 0 0 16px; color: #e8ecf2;">📦 Items ordered</h2>
        <table style="width: 100%; border-collapse: collapse; font-size: 14px;" cellpadding="0" cellspacing="0">
          <thead>
            <tr style="color: #8892a6; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em;">
              <th style="text-align: left; padding: 8px 12px; border-bottom: 1px solid #1a1f2e;">Product</th>
              <th style="text-align: center; padding: 8px 12px; border-bottom: 1px solid #1a1f2e;">Qty</th>
              <th style="text-align: right; padding: 8px 12px; border-bottom: 1px solid #1a1f2e;">Price</th>
            </tr>
          </thead>
          <tbody>${itemRows}</tbody>
        </table>
        <div style="text-align: right; margin-top: 16px; padding-top: 12px; border-top: 1px solid #1a1f2e;">
          <span style="color: #8892a6; font-size: 13px;">Delivery: </span>
          <span style="color: ${shippingColor}; font-weight: 600;">${shippingLabel}</span>
          <br/>
          <span style="font-size: 18px; font-weight: 700; color: #00E5CC;">Total: ${formatPrice(totalPrice)}</span>
        </div>
      </div>

      <div style="background: #0d1117; border: 1px solid #1a1f2e; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
        <h2 style="font-size: 16px; margin: 0 0 16px; color: #e8ecf2;">🚚 Delivery address</h2>
        <table style="font-size: 14px; color: #b0b8c8;" cellpadding="4" cellspacing="0">
          <tr><td style="color: #8892a6; padding-right: 12px;">Name:</td><td><strong style="color: #e8ecf2;">${escapeHtml(shipping.firstName)} ${escapeHtml(shipping.lastName)}</strong></td></tr>
          <tr><td style="color: #8892a6; padding-right: 12px;">Email:</td><td>${escapeHtml(shipping.email)}</td></tr>
          <tr><td style="color: #8892a6; padding-right: 12px;">Phone:</td><td>${escapeHtml(shipping.phone)}</td></tr>
          <tr><td style="color: #8892a6; padding-right: 12px;">Address:</td><td>${escapeHtml(shipping.address)}</td></tr>
          <tr><td style="color: #8892a6; padding-right: 12px;">City:</td><td>${escapeHtml(shipping.city)}, ${escapeHtml(shipping.county)}</td></tr>
          <tr><td style="color: #8892a6; padding-right: 12px;">Country:</td><td>${escapeHtml(shipping.country)}${shipping.postalCode ? `, ${escapeHtml(shipping.postalCode)}` : ''}</td></tr>
          <tr><td style="color: #8892a6; padding-right: 12px;">Payment:</td><td>${paymentLabel}</td></tr>
        </table>
      </div>
