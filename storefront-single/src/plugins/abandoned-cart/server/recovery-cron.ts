...
import { absoluteUrl, getMarketConfig, type MarketKey } from '@/lib/market';
...
async function sendOne(to: string, email: RecoveryEmail): Promise<string | undefined> {
  if (isDryRun()) {
    console.log(`[recovery-email DRY_RUN] would send "${email.subject}" to ${to}`);
    return 'dry-run';
  }
  const result = await sendEmail({
    from: getMarketConfig().contact.fromEmail,
    to: [to],
...
function recoveryUrlFor(cart: CartDoc): string {
  ...
  return absoluteUrl(`/recover/${token}`);
}
...
async function advanceCartToStep(
  ...
  try {
    ...
    const messageId = await sendOne(cart.email, email);
    await carts.updateOne(
      { cartId: cart.cartId },
      {
        $push: {
