...
// Re-bound after each `mongoMock.reset()` because reset drops the collection
// objects and creates fresh mocks on next access.
let mockFindOne: ReturnType<typeof mongoMock.collection>['findOne'];
let mockFindOneAndUpdate: ReturnType<typeof mongoMock.collection>['findOneAndUpdate'];
let mockUpdateOne: ReturnType<typeof mongoMock.collection>['updateOne'];

const baseShipping = buildShipping({ country: 'RO' });

beforeEach(() => {
  mongoMock.reset();
  const orders = mongoMock.collection(ORDERS_COLLECTION);
  mockFindOne = orders.findOne;
  mockFindOneAndUpdate = orders.findOneAndUpdate;
  mockUpdateOne = orders.updateOne;
});
...
describe('transitionStatus', () => {
  ...
  it('returns illegal-transition when filter does not match and order exists', async () => {
    ...
    mockFindOne.mockResolvedValueOnce({ orderId: 'X', status: 'cancelled' });
    const result = await transitionStatus('X', 'paid');
...
describe('editShipping', () => {
  ...
  it('refuses on terminal statuses', async () => {
    mockFindOneAndUpdate.mockResolvedValueOnce(null);
    mockFindOne.mockResolvedValueOnce({ orderId: 'X', status: 'refunded' });
