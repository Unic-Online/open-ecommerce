...
// the real registry shape (see AGENTS.md: content/products is the single
// source of truth).
const validBody = {
  slug: 'oslo-nightstand',
  name: 'Maria T.',
  rating: 5,
  text: 'Excellent product, highly recommend it to everyone.',
};
...
describe('POST /api/reviews', () => {
  ...
  it('degrades to unverified when the token names a different slug than the one submitted', async () => {
    ...
    const res = await call({ ...validBody, slug: 'oslo-nightstand', rt: token });
    expect(res.status).toBe(200);
