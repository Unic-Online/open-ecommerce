...
describe('RevolutPaymentWidgets', () => {
  ...
  it('keeps wallet buttons disabled until shipping is valid', async () => {
    ...
    expect(
      screen.getByText(/enter your delivery address to activate revolut pay/i)
    ).toBeInTheDocument()
  ...
  it('does not create a card order on mount even when shipping is valid (#14)', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /pay.*1[,.]709.*card/i })
    ).toBeInTheDocument()
  })

  it('creates the card session only after the explicit Pay by card click', async () => {
    render(<RevolutPaymentWidgets {...makeProps()} />)
  ...
  it('configures Revolut Pay for validated mobile redirects', async () => {
    ...
    expect(options).toMatchObject({
      mobileRedirectUrls: {
        success: 'https://shop.example.com/revolut-pay/return/success',
        failure: 'https://shop.example.com/revolut-pay/return/failure',
        cancel: 'https://shop.example.com/revolut-pay/return/cancel',
      },
      customer: {
        name: 'Ion Popescu',
        email: 'ion@test.ro',
