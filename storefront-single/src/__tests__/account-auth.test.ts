...
describe('account-auth', () => {
  ...
  describe('magic-link token', () => {
    it('roundtrips email + nonce', () => {
      const token = createMagicLinkToken('ion@test.ro', 'nonce-abc-123');
    ...
    it('lowercases the email at issue time', () => {
      const token = createMagicLinkToken('Ion@Test.RO', 'n1');
      const decoded = verifyMagicLinkToken(token);
      expect(decoded?.email).toBe('ion@test.ro');
    });

    it('roundtrips the nonce', () => {
      const token = createMagicLinkToken('john@test.com', 'n2');
      const decoded = verifyMagicLinkToken(token);
      expect(decoded?.nonce).toBe('n2');
    });
    ...
    it('rejects an expired token', () => {
      ...
      try {
        ...
        const token = createMagicLinkToken('ion@test.ro', 'n5');
  ...
  describe('cross-kind isolation', () => {
    it('magic-link token is rejected by the session verifier', () => {
      const magic = createMagicLinkToken('ion@test.ro', 'nonce-x');
