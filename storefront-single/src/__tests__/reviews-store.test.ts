...
const { mockIsDryRun, mockGetReviewsByLocale } = vi.hoisted(() => ({
  ...
  mockGetReviewsByLocale: vi.fn((slug: string) => {
    if (slug === 'oslo-nightstand') {
      return [
        {
          id: 'static-1',
          ...
          rating: 5,
          title: 'Great piece',
          text: 'A clean, curated static review.',
...
describe('insertPendingReview', () => {
  const baseInput = {
    ...
    name: 'Maria T.',
    rating: 5 as const,
    text: 'Excellent product, highly recommend it to anyone.',
...
describe('getApprovedReviews', () => {
  it('maps approved docs to the display Review shape, falling back to a generic title', async () => {
    const id = new ObjectId();
    reviews().cursor.toArray.mockResolvedValueOnce([
      {
        _id: id,
    ...
    const result = await getApprovedReviews('oslo-nightstand');
    ...
    expect(reviews().find).toHaveBeenCalledWith({ slug: 'oslo-nightstand', status: 'approved' });
  });
  ...
  it('uses the submitted title when present, instead of the fallback', async () => {
    ...
    const [result] = await getApprovedReviews('oslo-nightstand');
    expect(result.title).toBe('Recommend it');
  });
...
describe('getMergedReviews', () => {
  it('puts DB-approved reviews first, then the static corpus when reviewsKey is set', async () => {
    reviews().cursor.toArray.mockResolvedValueOnce([]);
    const result = await getMergedReviews('oslo-nightstand', 'oslo-nightstand');
    expect(mockGetReviewsByLocale).toHaveBeenCalledWith('oslo-nightstand');
  ...
  it('skips the static lookup when reviewsKey is absent, but always attempts DB reviews', async () => {
    reviews().cursor.toArray.mockResolvedValueOnce([]);
    const result = await getMergedReviews('unknown-slug', undefined);
