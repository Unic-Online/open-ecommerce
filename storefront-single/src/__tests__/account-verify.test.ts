 ...
 *  - On success: sets sf_account_session cookie + 302 redirect to /account.
 *  - On failure: 302 redirect to /account?error=invalid (or 400).
 */
...
describe('/api/account/verify', () => {
  ...
  it('redirects to /account on a fresh, valid token and sets a session cookie', async () => {
    const token = createMagicLinkToken('ion@test.ro', 'fresh-nonce');
  ...
  it('redirects to /account on a fresh EN token', async () => {
    const token = createMagicLinkToken('john@test.com', 'en-nonce');
    const res = await GET(makeRequest(token));
    expect(res.headers.get('location')).toMatch(/\/account$/);
  });

  it('rejects a re-used token (nonce already consumed)', async () => {
    const token = createMagicLinkToken('ion@test.ro', 'used-nonce');
    mockConsumeToken.mockResolvedValueOnce(null);
    const res = await GET(makeRequest(token));
    expect(res.status).toBe(307);
    expect(res.headers.get('location')).toMatch(/\/account\?error=/);
    expect(res.headers.get('set-cookie')).toBeNull();
  });

  it('rejects a tampered token', async () => {
    const token = createMagicLinkToken('ion@test.ro', 'tampered-nonce');
  ...
  it('rejects an expired token', async () => {
    ...
    try {
      ...
      const token = createMagicLinkToken('ion@test.ro', 'expired-nonce');
  ...
  it('email used to issue session matches the email from the consumed nonce, not from the token payload', async () => {
    ...
    const token = createMagicLinkToken('ion@test.ro', 'mismatch-nonce');
