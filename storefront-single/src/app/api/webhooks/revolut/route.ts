...
export async function POST(request: Request) {
  ...
  // safety net for the card→cash-on-delivery switch — a late `ORDER_COMPLETED` webhook
  // for the orphaned card session must NOT promote a cash-on-delivery doc to `paid`.
  //
  ...
  const orderMarketConfig = getMarketConfig()
  ...
  try {
    await Promise.all([
      sendEmail({
        ...
        subject: `Order paid #${orderIdStr} — ${orderMarketConfig.name}`,
        html: renderOrderEmail({ orderId: orderIdStr, items, shipping, shippingCost, totalPrice: total, paymentMethod: 'card' }),
        replyTo: shipping.email,
      }),
      sendEmail({
        from: orderMarketConfig.contact.fromEmail,
        to: [shipping.email],
        subject: customerOrderEmailSubject('paid', orderIdStr),
  ...
  after(async () => {
    try {
      const result = await sendServerPurchase({
        ...
        // market config currency for legacy docs.
        ...
        shippingCost,
        eventSourceUrl: absoluteUrl(`/order-confirmation/${orderIdStr}`),
        tracking,
