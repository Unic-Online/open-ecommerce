...
type PaymentMethodParam = 'cod' | 'card' | 'all';
type MarketParam = 'main' | 'all';
...
const PAYMENT_METHOD_OPTIONS: ReadonlyArray<PaymentMethodParam> = ['all', 'cod', 'card'];
const MARKET_OPTIONS: ReadonlyArray<MarketParam> = ['all', 'main'];
      ...
      {kpis.map((k) => (
        <div className={styles.card} key={k.currency}>
          ...
          <span className={styles.cardSub}>
            {formatMoney(k.revenue, k.currency)} · avg{' '}
            {formatMoney(Math.round(k.avgOrder), k.currency)}
          </span>
        </div>
      ))}
    </div>
