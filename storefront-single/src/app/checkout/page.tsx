...
import { Link, useRouter, hrefFor } from '@/lib/nav';
...
type SelectedPaymentMethod = 'card' | 'cod';
...
export default function CheckoutPage({ forcedCheckoutPaymentVariant }: CheckoutPageProps = {}) {
  ...
  // Some markets (e.g. FR) are card-only — don't surface the cod option.
  const codAllowed = market.checkout.paymentMethods.includes('cod');
  ...
  }, [items, totalItems, totals.total, market.currency]);
  ...
  function handlePaymentSuccess(orderId: string) {
    ...
    router.push(hrefFor({ pathname: '/order-confirmation/[orderId]', params: { orderId } }));
  }
  ...
  async function handleCod() {
    ...
    try {
      ...
      clearAppliedCoupon();
      clearCart();
      router.push(hrefFor({ pathname: '/order-confirmation/[orderId]', params: { orderId: data.orderId } }));
    } catch (err: unknown) {
  ...
  if (items.length === 0 && !codSubmitting) {
    return (
      <div className={styles.emptyWrap}>
        <div className="container">
          <div className={styles.emptyInner}>
            ...
            {step === 'payment' && (
              ...
              >
                ...
                {/* Compact bullet list is the default everywhere now (#14).
                    Customer picks card vs cash-on-delivery explicitly; nothing fires
                ...
                {codAllowed && (
                  <fieldset className={styles.compactPaymentFieldset}>
                    ...
                    <div className={styles.compactPaymentOptions}>
                      ...
                      <label
                        className={`${styles.compactPaymentOption} ${
                          selectedPaymentMethod === 'cod' ? styles.compactPaymentOptionActive : ''
                        }`}
                      >
                        <input
                          type="radio"
                          ...
                          className={styles.compactPaymentRadio}
                          data-testid="checkout-pay-method-cod"
                        />
                        <span className={styles.compactPaymentContent}>
                          <span className={styles.compactPaymentTitleRow}>
                            <span className={styles.compactPaymentName}>{t('payment.cod')}</span>
                            <span className={styles.compactPaymentMeta}>{t('payment.codMeta')}</span>
                          </span>
                          <span className={styles.compactPaymentPrice}>{fmt(totals.total)}</span>
                        </span>
                      </label>
                ...
                {selectedPaymentMethod === 'card' ? (
                  ...
                  <div className={styles.compactPaymentAction}>
                    <button
                      type="button"
                      className={`${styles.codBtn} ${styles.compactCodBtn}`}
                      onClick={handleCod}
                      disabled={codSubmitting}
                      data-testid="checkout-confirm-cod"
                    >
                      {codSubmitting
                        ? t('actions.processing')
                        : t('actions.confirmCod', { total: fmt(totals.total) })}
                    </button>
