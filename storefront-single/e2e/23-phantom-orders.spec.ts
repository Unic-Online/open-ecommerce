 ...
 *     cash-on-delivery (cod) or abandoned. Combined with /api/order also
 *     creating a fresh doc, cod-after-card-prep produced two rows in Mongo.
 *
 ...
 *      "Pay by card" click.
 *   3. Switching from card to cod does NOT trigger a second card-prep.
 *
...
test.describe('Phantom orders regression (#14)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Mock /api/order so the cod path doesn't depend on Resend.
  ...
  test('reaching payment step does not auto-create a Revolut order', async ({ page }) => {
    ...
    // render its explicit "Pay by card" CTA — the exact state the old
    // auto-prepare effect used to POST from — then assert it never did.
    await expect(
      page.getByRole('button', { name: /pay .+ by card/i }),
    ).toBeVisible();
    expect(createOrderRequests).toHaveLength(0);
  });

  test('Revolut order is created only after the explicit Pay by card click', async ({ page }) => {
  ...
  test('switching from card to cod does not trigger another card-prep call', async ({ page }) => {
    const createOrderRequests: string[] = [];
    page.on('request', (req) => {
      if (req.url().includes('/api/payments/revolut/create-order')) {
        createOrderRequests.push(req.url());
    ...
    // Customer changes their mind: pick cod without ever clicking card.
