...
const PRODUCT_PATH = '/furniture/oslo-nightstand';
...
test.describe('Review moderation — submit, queue, approve, publish', () => {
  ...
  const reviewText = 'Automated end-to-end test review for the moderation flow.';
  ...
  test('a fresh submission is pending — invisible on the product page', async ({ page }) => {
    ...
    await page.getByLabel(/your name/i).fill(reviewerName);
    await page.getByLabel(/^comment$/i).fill(reviewText);
