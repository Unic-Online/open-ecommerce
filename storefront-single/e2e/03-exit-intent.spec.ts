...
// Exit-intent specs run on the default storefront (`/`). The single `main`
// market prices in EUR, and the seeded EUR item survives the CartProvider
// price guard.
...
test.describe('Phase 1 — exit-intent popup', () => {
  ...
  async function expectPopupNeverShows(
    ...
    const dialog = page.getByRole('dialog', {
      name: /weren't leaving without your basket/i,
    });
    await expect(async () => {
      await fire();
  ...
  test('fires from a real desktop top-edge mouse leave', async ({ page }) => {
    ...
    await expect(
      page.getByRole('dialog', { name: /weren't leaving without your basket/i }),
  ...
  test('submitting the form persists the email and shows success state', async ({ page }) => {
    ...
    const input = dialog.getByPlaceholder(/enter your email address here/i);
    ...
    // After submit, the dialog's labelledby h2 changes ("Thank you! 🎉"), so
...
test.describe('Mobile — exit-intent triggers', () => {
  ...
  const dialogName = /weren't leaving without your basket/i;
  ...
  test('back from /checkout re-arms; the following back press intercepts', async ({ page }) => {
    ...
    // Real journey to checkout: open the basket sidebar, tap "Proceed to
    // checkout" (router.push — a client-side navigation on top of the sentinel).
    await page.getByTestId('cart-toggle').click();
    await page.getByTestId('cart-checkout').click();
    await expect(page).toHaveURL(/\/checkout/);

    // Back #1: leaves checkout, lands back on the product page (the sentinel
    // entry). No popup — /checkout is a skip path and this hop stays on-site.
    await page.goBack();
    await expect(page).toHaveURL(/\/furniture\/oslo-nightstand/);
  ...
  test('back press with an empty cart is NOT intercepted (no sentinel armed)', async ({ page }) => {
    ...
    await page.goBack();
    // No sentinel → back leaves the product page normally, landing on home.
